...
int dem_SoLuongCuaHangCoTaiThanhPho(DaySo ds, char *str);
void xoa_MotCuaHang(DaySo &ds, int pos);
void xoa_ThongTinTatCaCuaHangThuocPhuong(DaySo &ds, char *str);
...
int dem_SoLuongCuaHangCoSoNhaLonHon100(List list);
...
int main() {
  ...
  char str[30];
  cout << "\nNhap ten thanh pho can dem so luong cua hang: ";
  cin.getline(str, 30);
  int ketqua = dem_SoLuongCuaHangCoTaiThanhPho(ds, str);
  cout << "So luong cua hang co tai thanh pho " << str << " la " << ketqua << endl;

  cout << "\nNhap phuong can xoa thong tin tat ca cua hang: ";
  cin.getline(str, 10);
  xoa_ThongTinTatCaCuaHangThuocPhuong(ds, str);
  cout << "Danh sach tat ca cua hang sau khi xoa la: ";
  xuat_DanhSachCacCuaHang(ds);
  ...
  int ketqua2 = dem_SoLuongCuaHangCoSoNhaLonHon100(ds1);
  cout << "\nSo luong cua hang co so nha > 100 la: " << ketqua2;
...
int dem_SoLuongCuaHangCoTaiThanhPho(DaySo ds, char *str) {
  int dem = 0;
  for (int i = 0; i < ds.n; i++)
    if (STRCMP(ds.data[i].thanhPho, str) == 0)
      dem++;
  return dem;
}
