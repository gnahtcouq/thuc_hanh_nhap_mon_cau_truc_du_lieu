...
void xuatTenHangTenButCoSoLuongKhongTonKho(Nodeptr list);
void sapXepGiamDanTheoSoLuong(Nodeptr &list);
...
int main() {
    ...
    // Phan 1 - Cau 2
    cout << "\nTen Hang SX va Ten But co so luong ton kho bang 0" << endl;
    xuatTenHangTenButCoSoLuongKhongTonKho(ds);
...
void xuatDanhSachCayBut(Nodeptr list) {
    ...
    cout << setw(15) << left << "Don gia"
         << "\t";
    cout << setw(30) << left << "Hang san xuat"
         << "\t" << endl;
    while (list != NULL) {
...
void xuatTenHangTenButCoSoLuongKhongTonKho(Nodeptr list) {
    if (list != NULL) {
        ...
        while (p != NULL) {
            if (p->data.soLuong == 0) {
                cout << "\tTen hang " << p->data.hangSX << ", ten but "
                     << p->data.tenCB << endl;
            }
            p = p->link;
        }
...
void sapXepGiamDanTheoSoLuong(Nodeptr &list) {
    for (Nodeptr pTmp = list; pTmp != NULL; pTmp = pTmp->link)
        for (Nodeptr pTmp2 = pTmp->link; pTmp2 != NULL; pTmp2 = pTmp2->link)
            if (pTmp->data.soLuong < pTmp2->data.soLuong) {
...
void chenSauNode(QUEUE &q, int x, int pos) {
    ...
    enQueue(tam, x);
    while (!isEmpty_Queue(p)) enQueue(tam, deQueue(p));
    q = tam;
}
